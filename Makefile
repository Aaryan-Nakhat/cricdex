...
docker-newsletter:
	docker compose run --rm cricdex uv run python scripts/newsletter.py --collection "$${COLLECTION:-ipl}"

docker-pulse-fetch:
	docker compose run --rm cricdex uv run python scripts/pulse.py fetch --period "$${PERIOD:-week}" --limit "$${LIMIT:-50}"
