...
| `matchups` / `phase` / `form` / `partnerships` | Batter-vs-bowler head-to-heads + pace/spin splits; powerplay / middle / death specialist boards; recent-form-vs-career deltas (direction-corrected); batter-pair stands — all four surfaces, reading the same exported JSON, with the full player filter bar. |
...
| `dashboard` | Streamlit app mirroring the web pages: Leaderboards, Player Profile (Wikidata photo + DOB + dismissal fingerprint + style twins), Compare, Head-to-head, Matchups, Phase, Form, Partnerships, Scout, Auction, Records, Venues, Update Data. |
